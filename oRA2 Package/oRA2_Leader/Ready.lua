...
local revision = tonumber(("$Revision: 670 $"):match("%d+"))
...
function mod:READY_CHECK_CONFIRM(unitId, confirm)
	-- only raid leader / officers will receive this
	if not ready then ready = self:new() end
	local name = UnitName(unitId)
	if confirm then -- explicit check for 1 since it's 1 / 0
		ready[name] = "ready"
	else
...
function mod:READY_CHECK_FINISHED(someBoolean)
