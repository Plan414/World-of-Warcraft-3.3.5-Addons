...
function DrDamage:PlayerData()
	...
	(GetSpellInfo(18223)), (GetSpellInfo(15407)), (GetSpellInfo(29703)), (GetSpellInfo(68766)), (GetSpellInfo(50434))
	}
--GENERAL
...
--TALENTS
	...
	self.Calculation["Custom Mind Mastery"] = function( calculation, talentValue, Talents )
		local baseValue = self.BaseTalents["Mind Mastery"]
		if baseValue ~= talentValue then
			calculation.spellDmg = calculation.spellDmg + 0.03 * (talentValue - baseValue) * calculation.int
		end
	end
	self.Calculation["Custom Netherwind Presence"] = function( calculation, talentValue, Talents )
		local baseValue = self.BaseTalents["Netherwind Presence"]
		if baseValue ~= talentValue then
...
--ABILITIES
	...
	self.Calculation["Summon Water Elemental"] = function( calculation, ActiveAuras, Talents, _ , baseSpell )
		...
		if ActiveAuras["Totem of Wrath"] and SP < 280 then
			SP = 280
		elseif SP < 80 and UnitBuff("player",DS) then
			SP = 80
		end
...
--Custom
	...
	--ARCANE:
		...
		[GetSpellInfo(31584)] = { 	[1] = { Effect = 0.03, Spells = "Frostfire Bolt", ModType = "Mind Mastery" }, 
									[2] = { Effect = 1, Spells = "All", ModType = "Custom Mind Mastery" }, Manual = "Mind Mastery" },
		--Netherwind Presence
		[GetSpellInfo(44400)] = {	[1] = { Effect = 1, Spells = "All", ModType = "Custom Netherwind Presence" }, Manual = "Netherwind Presence" },
		--Spell Power
