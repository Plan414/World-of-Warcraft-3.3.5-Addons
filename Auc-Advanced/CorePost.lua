--[[
	...
	Revision: $Id: CorePost.lua 4696 2010-03-24 22:04:27Z Nechckn $
...
local ServerNeutralDeposit = .01 -- Default = .25
local ServerFactionDeposit = .01 -- Default = .05
...
function GetDepositCost(item, duration, faction, count)
	...
	if faction == "neutral" or faction == "Neutral" then faction = ServerNeutralDeposit else faction = ServerFactionDeposit end --DEPOSIT FIX HERE if realmName = ChromieCraft then faction = ChromieCraftDrposit else
	count = count or 1
...
AucAdvanced.RegisterRevision("$URL: http://svn.norganna.org/auctioneer/branches/5.8/Auc-Advanced/CorePost.lua $", "$Rev: 4696 $")
