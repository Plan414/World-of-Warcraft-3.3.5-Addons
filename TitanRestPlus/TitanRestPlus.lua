--[[
	...
	Author: until 2.0.2 GrayElf, 3.0.1 by Maillen, 3.0.2 and later by Vogonjeltz
...
TITAN_RESTPLUS_MENU_TEXT = "Rest+";  -- @@ Vogonjeltz
...
function RestPlus_OnLoad(self)
	u = Utility_Class:New();
	-- @@ Changed this to self @@ Vogonjeltz
...
function TitanPanelRestPlusButton_GetButtonText(id)
	...
	if (playerLevel and playerLevel < 80) then -- EMERALD
	...
	for index,value in pairs(RestPlus_Data) do
		...
		if(Level == 80) then
			...
			if(RestPlus_Settings.ShowState) then
				tempstr = tempstr..RESTPLUS_LEVEL80;
			end
