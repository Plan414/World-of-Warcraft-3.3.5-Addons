...
function Manage:PostItems(itemID)
	...
	local leavingInBag = self:GetConfigValue(itemID, "leavingInBag")
	local threshold = self:GetConfigValue(itemID, "threshold")
	...
	-- Check leaveInBag
	leaveQty = GetItemCount(itemID) - (auctionsCreated * perAuction)
	if ( leaveQty < leavingInBag ) then
	    local leaveStacksCount = 1
	    auctionsCreated = auctionsCreated - 1
	    while ( (leaveStacksCount * perAuction) < leavingInBag ) do
	        leaveStacksCount = leaveStacksCount + 1
	        auctionsCreated = auctionsCreated - 1
	    end
	    leaveQty = GetItemCount(itemID) - (auctionsCreated * perAuction)
    	--QuickAuctions:Log(name.."2", string.format("Skipped |cffffff20%d|r stacks, leaving |cff20ff20%d|r items in bags (Min qty in bags is |cffff2020%d|r)", leaveStacksCount, leaveQty, leavingInBag))
    	if ( auctionsCreated <= 0 ) then
    	    return
	    end
	end
    ...
    if( auctionsCreated < postCap ) then
		QuickAuctions:Log(name, string.format(L["Queued %s to be posted (Cap is |cffff2020%d|r, only can post |cffff2020%d|r need to restock)"], itemLink, postCap, auctionsCreated))
	end
