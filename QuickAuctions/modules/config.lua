	...
	quantity = {
		...
		args = {
			desc = {
				order = 1,
				type = "description",
				name = function(info) return string.format("Will post at most |cfffed000%d|r auctions in stacks of |cfffed000%d|r leaving in bag |cfffed000%d|r items.", getGroupSetting("postCap", info[2]), getGroupSetting("perAuction", info[2]), getGroupSetting("leavingInBag", info[2])) end,
				hidden = hideHelpText,
			},
			header = {
				order = 2,
			...
			overrideLeavingInBag = {
				order = 7,
				type = "toggle",
				name = "Enable leaving in bag",
				desc = "Allows you to set leaving in bag settings for this group.",
				hidden = hideForDefault,
				disabled = false,
				set = setGroupOverride,
				get = getGroupOverride,
			...
			leavingInBag = {
				...
				type = "range",
				name = "Leaving in Bag",
				desc = "How many items should be leave in bags, 0 will mean that all items will be posted.",
				min = 0, max = 20, step = 1,
			},
		},
	},
