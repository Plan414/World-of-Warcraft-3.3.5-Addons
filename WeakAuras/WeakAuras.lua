...
groupFrame:SetScript("OnEvent", function(self, event)
  ...
  if IsInRaid() then
    ...
    --	  playerName = playerName:gsub("-", " - ");		--not needed for non-cross/connected-realms
      guid = UnitGUID(uid);
      if (guid) then
...
function WeakAuras.ScanForLoads(self, event, arg1)
  ...
  local player, zone, spec, role = UnitName("player"), GetRealZoneText(), GetActiveTalentGroup(), UnitGroupRolesAssigned("player");
  if role == "NONE" then
    if IsInRaid() then
      for i=1,GetNumGroupMembers() do
